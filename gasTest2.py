...
while (True):
    file = open("depletion211007A.csv","a")
    count = 0
    samplingTime = 10
    FEvalues = [0]*4
    FNvalues = [0]*4
    ...
    print(datetime.now(),NO_AE,NO_WE,NO2_AE,NO2_WE)

    file.write(str(datetime.now()))
    file.write(",")
    file.write(str(NO_AE))
    file.write(",")
    file.write(str(NO_WE))
    file.write("\n")
    file.close()

    futureTime = datetime.now() + timedelta(milliseconds=10000)
    while (dt < futureTime):
            dt=datetime.now()
